...
describe('find', () => {
  describe('by id', () => {
    let uuidSimple,
      uuidSimpleRelationship,
      uuidRelationshipArray,
      uuidRelationshipArrayOfOneElement,
      simple = {
        _label: 'AghanimTest',
        stringProperty: 'test'
      },
    ...
    beforeEach(done => {
      ;(async () => {
        ...
        uuidRelationshipArrayOfOneElement = await aghanim.save(
          relationshipArrayOfOneElement
        )
        await aghanim.save({
          _label: 'AghanimTest',
    ...
    it('object with relationship array', done => {
      ;(async () => {
        ...
        expect(result.entities.map(e => e._label)).to.be.deep.equal([
          'AghanimTest',
          'AghanimTest',
          'AghanimTest'
        ])
        expect(result.entities.map(e => e.name).sort()).to.be.deep.equal([
          'Chaos Knight',
          'Enigma',
          'Io'
        ])
    ...
    it('object with single object relationship', done => {
      ;(async () => {
        ...
        expect(result)
          ...
          .that.is.an('object')
          .that.have.property('name', 'Elder Titan')
        done()
      })()
    })
  ...
  describe('by query object', () => {
    ...
    it('multiple labels', done => {
      ;(async () => {
        ...
        expect(result)
          .to.be.an('array')
          .that.have.lengthOf(data.length)
    ...
    it('order', done => {
      ;(async () => {
        ...
        expect(result)
          .to.be.an('array')
          .that.have.lengthOf(aghanimTestLength)
    ...
    it('skip', done => {
      ;(async () => {
        ...
        expect(result)
          .to.be.an('array')
          .that.have.lengthOf(aghanimTestLength - 2)
    ...
    it('order include', done => {
      ;(async () => {
        ...
        expect(result).all.to.have.property('uuid')
        expect(result).all.to.have.property('_label', 'AghanimTest')
        expect(_.map(result[0].skills, 'title')).to.be.deep.equal(
          _.chain(crystalMaiden.skills)
            .sortBy('title')
            .map('title')
            .value()
        )
        expect(_.map(result[1].skills, 'title')).to.be.deep.equal(
          _.chain(sven.skills)
            .sortBy('title')
            .map('title')
            .value()
    ...
    it('chain where', done => {
      ;(async () => {
        let result = await aghanim.findByQueryObject({
          label: 'AghanimTest',
          where: (n, args) =>
            n.boolean == args.value ||
            n.number <= args.number ||
            n.name.includes(args.string),
          args: {
            value: true,
            number: 20,
            string: 'na'
    ...
    it('where include', done => {
      ;(async () => {
        ...
        expect(_.flatMap(result, 'skills')).to.include.something.with.property(
          'title',
          'Crystal Nova'
        )
        expect(result).all.to.have.property('uuid')
