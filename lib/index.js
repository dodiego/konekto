...
let statements = cypherMapper.jsonMapper({
  _label: 'omegalul',
  lul: 'xd',
  so: {
    _label: 'buldog',
...
async function run () {
  ...
  await client.query('SET graph_path = agens_graph')
  for (const statement of statements) {
    console.log(statement.cypher.replace(/\$v/g, '$'), statement.parameters)
    await client.query(
      statement.cypher.replace(/\$v/g, '$'),
      Object.values(statement.parameters)
    )
  }
}
