...
class Konekto {
  ...
  async findByQueryObject (queryObject, options = {}) {
    ...
    const response = await this.client.query(statement.query, statement.params)
    parser.on('read', async node => {
      if (node._wkt) {
        node._wkt = await this.client.query(
          `SELECT ST_AsGeoJSON(geom)::json FROM public.geometries WHERE node_id = ${node._id}`
        )
      }
    })
  ...
  async findById (id, options) {
    const parser = new Parser()
    // SELECT ST_AsGeoJSON(geom)::json FROM public.geometries WHERE node_id = $1::text
    // SELECT ST_AsGeoJSON(geom)::json as geom from public.geometries inner join (MATCH (v1) WHERE id(v1) = $1 WITH v1\nOPTIONAL MATCH(v1)- [r * 0..] -> (v2) \nRETURN v1, r, v2) on public.geometries.node_id = id(m)::text
    const statement = {
      query:
        "MATCH (v1) WHERE id(v1) = $1 WITH v1\nOPTIONAL MATCH(v1)- [r*0..] -> (v2) \nRETURN v1, r, v2, (SELECT json_agg(json_build_object('geom', ST_AsText(geom), 'label', label, 'node_id', node_id )) FROM public.geometries WHERE node_id IN (id(v1)::text, id(v2)::text)) as geom",
      params: [id]
    }
    const response = await this.client.query(statement.query, statement.params)
  ...
  async deleteByQueryObject (queryObject) {
    const parser = new Parser()
    const statement = parser.jsonToCypherDelete(queryObject)
