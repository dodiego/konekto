...
async function _createSchema (json, client) {
  ...
  const createRelationshipNames = Promise.all(
    relationshipNames.map(vlabel => client.query(`CREATE ELABEL IF NOT EXISTS ${vlabel}`))
  )
  const createLabels = Promise.all(nodeLabels.map(vlabel => client.query(`CREATE VLABEL IF NOT EXISTS ${vlabel}`)))
