  ...
  async save (json, options = {}) {
    return _handleTransaction(
      async parser => {
        ...
        const result = await this.client.query(statement.query, statement.params)
  ...
  async findById (id, options) {
    return _handleTransaction(
      async parser => {
        ...
        statement.query = parser.getFinalQuery({}, statement.query, ['v1', 'v2'])
        const result = await _handleParseRows(parser, this.client, statement, options)
        return result[0]
      },
      'findById',
