...
class Konekto {
  ...
  setSqlMappings (mappings) {
    this.sqlMappings = {
      insert: mappings,
      delete: mappings,
      read: Object.values(mappings).map(m => ({ table: m.table, columns: m.mappings.map(x => x.column) }))
    }
  }
  ...
  async findByQueryObject (queryObject, options = {}) {
    return _handleTransaction(
      async parser => {
        const statement = await parser.jsonToCypherRead({ ...queryObject, _sql: this.sqlMappings.read }, options)
  ...
  async deleteByQueryObject (queryObject, options = {}) {
    return _handleTransaction(
      async parser => {
        ...
        const promise = new Promise(resolve => parser.on('readFinish', resolve))
        const result = await _handleParseRows(parser, this.client, statement, options)
        await promise
        if (result.length) {
