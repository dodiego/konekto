...
class Aghanim {
  ...
  async deleteByQueryObject (queryObject) {
    let statements = utils.jsonToCypherDelete(queryObject)
    await Promise.map(statements, statement => this.client.query(statement.query, statement.params))
    return true
  }
  ...
  async createGraph (graphName) {
    await this.client.query(`CREATE GRAPH ${graphName}`)
  }
