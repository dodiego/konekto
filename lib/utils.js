...
function getWhere (params, json, variableName) {
  ...
  if (json[where]) {
    let parsed = mongoParser.parse(json[where]).map((key, part) => {
      for (const [ operator, value ] of Object.entries(part)) {
        if (isPrimitive(value)) {
          let paramIndex
    ...
    console.log(parsed)
    whereQuery.push(sqlFromMongo(parsed, variableName).replace(/"/g, ''))
  }
