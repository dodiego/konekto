  ...
  if (graph.root._id) {
    if (options.hooks && options.hooks.beforeUpdate) {
      if (!(await options.hooks.beforeUpdate(graph.root, graph.rootObject))) {
        throw new Error(`beforeUpdate hook didn't return truthy value for node ${JSON.stringify(graph.root, null, 2)}`)
      }
    }
    addPartialQuery(queries, `MATCH (v1:${graph.root._label}) WHERE id(v1) = ${graph.root._id} SET v1 = $1`, variables)
  } else {
    if (options.hooks && options.hooks.beforeCreate) {
      if (!(await options.hooks.beforeCreate(graph.root, graph.rootObject))) {
        throw new Error(`beforeCreate hook didn't return truthy value for node ${JSON.stringify(graph.root, null, 2)}`)
      }
    }
...
async function parseRows (rows, rootKey, options = {}) {
  const relationships = {}
  const nodes = {}
  const roots = {}
      ...
      if (column && column !== '[]') {
        ...
        for (const match of matches) {
          if (match.groups.node) {
            ...
            delete nodes[match.groups.id].konekto_id
          } else {
  ...
  if (options.hooks && options.hooks.beforeRead) {
    for (const node of Object.values(nodes)) {
      if (
        !(await options.hooks.beforeRead(nodes[node._id], options.graph && options.graph.objects[node.konekto_id])) &&
        nodes[node._id]
      ) {
        delete nodes[node._id]
      }
