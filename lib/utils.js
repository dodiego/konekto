...
function jsonToCypherWrite (json) {
  ...
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    ...
    if (nodes[r.to]._id) {
      ...
      let toLabel = nodes[r.to][label] ? `:${nodes[r.to][label]}` : ''
      addPartialQuery(
        queries,
...
function queryObjectToCypher (json, getQueryEnd) {
  ...
  if (!relationships.length) {
    let params = []
    let query = [ 'MATCH (v1)' ]
    let whereQuery = getWhereCypher(params, root, 'v1')
    let orderQuery = getOrderCypher(root, 'v1')
    let paginationQuery = getPaginationCypher(root, params)
    if (whereQuery) {
      query.push(whereQuery)
    }
    query.push(getQueryEnd(new Set([ 'v1' ])))
    if (orderQuery) {
      query.push(orderQuery)
    }
    if (paginationQuery) {
  ...
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    ...
    relationshipStatement.query += ` ${getWith(variables)}`
