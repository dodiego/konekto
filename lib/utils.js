...
const parentReference = Symbol('parent')
...
function isPrimitive (value) {
  return (
    typeof value !== 'object' ||
    (Array.isArray(value) &&
      (typeof value[0] !== 'object' || value[0] instanceof Date)) ||
...
async function jsonToCypherWrite (json, options = {}) {
  ...
  if (root._id) {
    addPartialQuery(
      queries,
      `MATCH (v1:${root._label}) WHERE id(v1) = ${root._id} SET v1 = $1`,
      variables
    )
  } else {
  ...
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    ...
    if (nodes[r.to]._id) {
      if (options.hooks && options.hooks.beforeUpdate) {
        ...
        if (!ok) {
          throw new Error(
            `beforeUpdate hook didn't return truthy value for node: ${JSON.stringify(
              indexesPerNode[r.to],
              null,
              2
            )}`
      ...
      if (options.hooks && options.hooks.beforeCreate) {
        ...
        if (!ok) {
          throw new Error(
            `beforeCreate hook didn't return truthy value for node: ${JSON.stringify(
              indexesPerNode[r.to],
              null,
              2
            )}`
...
function queryObjectToCypher (json, getQueryEnd) {
  ...
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    ...
    variables.add(`r${rIndex}`)
    variables.add(`v${toIndex}`)
    let relationshipMatchSufix = getMatchSufix(
      toNode,
      `v${toIndex}`,
      getWith(variables)
    )
    let relationshipStatement = {
      query: `MATCH (v${fromIndex})-[r${rIndex}:${r.name}]->(v${toIndex}) ${
        relationshipMatchSufix.query
      }`,
...
function parseRows (rows, rootKey) {
  ...
  for (const item of rows) {
    ...
    for (const column of Object.values(item)) {
      if (column && column !== '[]') {
        let matches =
          getMatches(column, nodeRegex) || getMatches(column, relationshipRegex)
  ...
  if (
    !(
      ...
      relationships.constructor === Object
    )
  ) {
