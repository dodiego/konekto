...
async function jsonToCypherWrite (json, options = {}) {
  const { nodes, relationships, root, objects, rootObject } = getNodesAndRelationships(json)
  const params = [root]
  ...
  if (root._id) {
    if (options.hooks && options.hooks.beforeUpdate) {
      if (!(await options.hooks.beforeUpdate(rootObject))) {
    ...
    if (options.hooks && options.hooks.beforeCreate) {
      if (!(await options.hooks.beforeCreate(rootObject))) {
  ...
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    ...
    if (nodes[r.to]._id) {
      if (options.hooks && options.hooks.beforeUpdate) {
        const ok = await options.hooks.beforeUpdate(objects[r.to])
        if (!ok) {
          throw new Error(
      ...
      if (options.hooks && options.hooks.beforeCreate) {
        const ok = await options.hooks.beforeCreate(objects[r.to])
        if (!ok) {
          throw new Error(
...
async function parseRows (rows, rootKey, options = {}) {
  ...
  if (options.hooks && options.hooks.beforeRead) {
    for (const node of Object.values(nodes)) {
      if (!(await options.hooks.beforeRead(nodes[node._id])) && nodes[node._id]) {
        delete nodes[node._id]
      }
    }
  }
