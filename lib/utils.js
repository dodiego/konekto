  ...
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    ...
    if (nodes[r.to]._id) {
      if (options.hooks && options.hooks.beforeUpdate) {
        const ok = await options.hooks.beforeUpdate(nodes[r.to])
        if (!ok) {
          continue
        }
      }
      ...
      if (options.hooks && options.hooks.beforeCreate) {
        const ok = await options.hooks.beforeCreate(nodes[r.to])
        if (!ok) {
          continue
        }
      }
...
function parseRows (rows, rootKey, options = {}) {
  ...
  if (options.hooks && options.hooks.beforeRead) {
    for (const node of Object.values(nodes)) {
      const { nodes: itemNodes } = getNodesAndRelationships(node)
      for (const json of itemNodes) {
        if (!options.hooks.beforeRead(json) && nodes[json._id]) {
          delete nodes[node._id]
        }
      }
    }
