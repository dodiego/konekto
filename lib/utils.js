...
function queryObjectToCypher (json, getQueryEnd) {
  ...
  for (const item of generator) {
    ...
    while (currentItem) {
      ...
      variables.add(`v${currentIndex}`)
      variables.add(`r${parentIndex}`)
      variables.add(`v${parentIndex}`)
      let match
      if (currentItem.key) {
      ...
      let relationshipWhere = getWhere(relationshipParams, currentChild, `v${currentIndex}`)
      if (relationshipWhere) {
        relationshipMatch.push(relationshipWhere)
      }
      relationshipMatch.push(variablesToCypherWith(variables, currentChild, relationshipParams))
      currentItem = nodes[currentItem.parent]
    }
    relationshipMatch.push(getQueryEnd(variables))
    statements.push({
  ...
  if (!hasRelationships) {
    statements.push({
      query: `${rootMatch.join(' ')} ${getQueryEnd(variables)}`,
      params: rootParams
    })
  }
