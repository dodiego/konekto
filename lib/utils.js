...
async function jsonToCypherWrite (json, options = {}) {
  ...
  if (root._id) {
    if (options.hooks && options.hooks.beforeUpdate) {
      if (!(await options.hooks.beforeUpdate(root, root))) {
    ...
    if (options.hooks && options.hooks.beforeCreate) {
      if (!(await options.hooks.beforeCreate(root, root))) {
  ...
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    ...
    if (nodes[r.to]._id) {
      if (options.hooks && options.hooks.beforeUpdate) {
        const ok = await options.hooks.beforeUpdate(nodes[r.to], root)
        if (!ok) {
          throw new Error(
      ...
      if (options.hooks && options.hooks.beforeCreate) {
        const ok = await options.hooks.beforeCreate(nodes[r.to], root)
        if (!ok) {
          throw new Error(
  ...
  if (options.hooks && options.hooks.beforeRead) {
    for (const node of Object.values(nodes)) {
      ...
      for (const json of Object.values(itemNodes)) {
        if (!(await options.hooks.beforeRead(json, root)) && nodes[json._id]) {
          delete nodes[node._id]
        }
      }
    }
