...
function queryObjectToCypher (json) {
  ...
  let statements = []
  statements.push({
    query: `${rootMatch.join(' ')} ${variablesToCypherWith([ 'v1' ], root, rootParams)} RETURN *`,
    params: rootParams
  })
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    const r = relationships[rIndex]
    ...
    let relationshipWhere = getWhere(relationshipParams, nodes[r.to], `v${toIndex}`)
    if (relationshipWhere) {
      relationshipMatch.push(relationshipWhere)
    }
    statements.push({
      query: `${rootMatch.join(' ')} WITH v1 ${relationshipMatch.join(' ')} ${variablesToCypherWith(
        [ 'v1', `r${rIndex}`, `v${toIndex}` ],
        nodes[r.to],
        relationshipParams
      )} RETURN *`,
