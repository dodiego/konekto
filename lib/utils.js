...
let order = Symbol('order')
let skip = Symbol('skip')
let limit = Symbol('limit')
...
function variablesToCypherWith (variables, json) {
  let query = []
  let variablesList = [ ...variables ]
  if (json) {
    if (json[order]) {
      if (json[order].startsWith('!')) {
        query.push(`ORDER BY ${variablesList[variablesList.length - 1]}.${json[order].slice(1)} DESC`)
      } else {
        query.push(`ORDER BY ${variablesList[variablesList.length - 1]}.${json[order]} ASC`)
      }
    }
    if (json[skip] > 0) {
      query.push(`SKIP ${json[skip]}`)
    }
    if (json[limit] > 0) {
...
function addPartialQuery (queries, query, variables, json) {
  queries.push(`${query} ${variablesToCypherWith(variables, json)}`)
}
...
function getWhere (params, json, variableName) {
  ...
  if (json[where]) {
    ...
    whereQuery.push(sqlFromMongo(clone, variableName).replace(/"/g, ''))
  }
...
function queryObjectToCypher (json) {
  ...
  let queries = []
  addPartialQuery(queries, rootMatch.join(' '), variables, root)
  for (let rIndex = 0; rIndex < relationships.length; rIndex++) {
    const r = relationships[rIndex]
    let fromIndex = indexesPerNode[r.from]
    let toIndex = indexesPerNode[r.to]
    ...
    variables.add(`r${rIndex}`)
    variables.add(`v${toIndex}`)
    let relationshipMatch = [ `OPTIONAL MATCH (v${fromIndex})-[r${rIndex}:${r.name}]->(v${toIndex})` ]
    let relationshipWhere = getWhere(params, nodes[r.to], `v${toIndex}`)
    if (relationshipWhere) {
      relationshipMatch.push(relationshipWhere)
    }
    addPartialQuery(queries, relationshipMatch.join(' '), variables, nodes[r.to])
  }
  return {
    variables,
    queries,
