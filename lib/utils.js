...
function queryObjectToCypher (json) {
  ...
  let rootWhere = getWhere(rootParams, root, 'v1')
  if (rootWhere) {
  ...
  for (const item of generator) {
    ...
    while (currentItem) {
      ...
      let parentIndex = baseIndex++
      variables.push(`v${currentIndex}`, `r${parentIndex}`, `v${parentIndex}`)
      let match
      if (currentItem.key) {
        match = `MATCH (v${currentIndex})-[r${parentIndex}:${currentItem.key}]->(v${parentIndex})`
        if (!currentItem.child.mandatory) {
      ...
      relationshipMatch.push(match)
      let relationshipWhere = getWhere(relationshipParams, currentItem.child, `v${currentIndex}`)
      if (relationshipWhere) {
        relationshipMatch.push(relationshipWhere)
      }
      relationshipMatch.push(variablesToCypherWith(variables, currentItem.child, relationshipParams))
      currentItem = nodes[currentItem.parent]
    }
    relationshipMatch.push('RETURN *')
    statements.push({
...
function jsonToCypherRead (json) {
  let { statements } = queryObjectToCypher(json)
  return statements
}
