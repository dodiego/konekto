...
function queryObjectIncludeToCypher (parentId, idGenerator, queryObject, withVariables, cypherParts = [], returnNames = []) {
  if (queryObject.include) {
    for (let include of queryObject.include) {
      if (typeof include.name !== 'string' || include.name.match(wordRegex).length > 1) {
...
function queryObjectToReadStatement (queryObject) {
  ...
  let {returnNames, cypherParts, nodeId} = queryObjectToMatchStatements(idGenerator, queryObject)
  let tailCypherParts = [
    new Statement(`RETURN ${returnNames.join(', ')}`)
  ]
  ...
  return reduceCypherParts([...cypherParts, ...tailCypherParts], {}, '\n')
}
