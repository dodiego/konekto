...
function _getFinalQuery (options, cypher, nodes) {
  ...
  if (options._sql) {
    ...
    for (const item of Object.values(sqlMappings)) {
      const out = item.mappings.map(m => `'${m.key}', ${item.table}.${m.column}`)
      if (options.projections && options.projections[item.table]) {
        const projection = options.projections[item.table]
        out.push(...Object.entries(projection).map(([key, column]) => `'${key}', ${column}`))
      }
      const columns = [`'konekto_id', ${item.table}.konekto_id`, ...out]
...
async function queryObjectToCypher (queryObject, options, eventEmitter, getQueryEnd) {
  ...
  finalStatement.query = _getFinalQuery(options, finalStatement.query, [...variables].filter(v => v.startsWith('v')))
