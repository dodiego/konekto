  ...
  if (json[where]) {
    ...
    const ast = await cypherParser.parse(
      `MATCH (${variableName}) WHERE ${json[where].filter
        ...
        .replace(/\s+:(\w+)\b/g, (a, b) => {
          if (typeof json[where].params[b] === 'string') {
            return `$${params.push(`"${json[where].params[b]}"`)}`
          } else {
            return `$${params.push(json[where].params[b])}`
          }
        })}`
...
async function getMatchSufix (json, variable, queryEnd) {
  ...
  if (sqlWhereQuery) {
    ...
    where.push(
      `(SELECT count(_id) FROM ${json[sqlWhere].table} WHERE _id = ${variable}->>'_id' AND ${sqlWhereQuery}) > 0`
    )
  }
  if (where.length) {
    where.unshift('WHERE')
