...
function getWhereCypher (params, json, variableName) {
  ...
  if (json.where) {
    ...
    const supportedFunctions = ['id', 'avg', 'sum', 'min', 'max', 'collect']
    const functionRegexes = supportedFunctions.map(s => `${s}\\(\\{?\\w+\\}?\\)`)
    const matches = {}
    const auxWhere = json.where.replace(new RegExp(functionRegexes.join('|')), match => {
      matches.xd = match
      return 'xd'
    })
    const parsedWhere = sqlWhereParser.parse(auxWhere, (operator, operands) => {
      let property
      let value
      if (operands.length === 2) {
        ...
        for (let i = 0; i < operands.length; i++) {
          ...
          if (operand.toString().includes('{this}')) {
            property = operand.replace('{this}', variableName)
          } else {
            value = operand
          }
        }
        if (isPrimitive(value)) {
          ...
          return `(${property} ${operator} $${paramIndex})`
        }
      }
    })
...
module.exports = class extends EventEmitter {
  ...
  jsonToCypherRead (json) {
    return queryObjectToCypher(json, variables => {
      const nodeIds = [...variables]
        .filter(v => v.startsWith('v'))
        .map(n => `id(${n})::text`)
        .join(',')

      return `
      RETURN *, (SELECT json_agg(json_build_object('geom', ST_AsText(geom), 'label', label, 'node_id', node_id )) 
      FROM public.geometries WHERE node_id IN (${nodeIds})) as geom`
    })
  }
