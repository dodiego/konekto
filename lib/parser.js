...
function getFinalQuery (queryObject, cypher, nodes) {
  const selectPart = ['json_agg(cypher.*) as cypher_info']
  const joinPart = []
  const joinFilter = nodes.map(n => `node_id = id(cypher.${n})`).join(' OR ')
  if (queryObject._sql) {
    const sqlColumns = []
    for (const sqlPart of queryObject._sql.parts) {
      const columns = sqlPart.columns.map(c => `'${c}', ${sqlPart.table}.${c}`)
...
async function queryObjectToCypher (queryObject, eventEmitter, getQueryEnd) {
  ...
  if (!relationships.length) {
    ...
    statement.query = getFinalQuery(queryObject, statement.query, ['v1'])
    return statement
  }
  ...
  finalStatement.query = getFinalQuery(queryObject, finalStatement.query, [...variables].filter(v => v.startsWith('v')))
  eventEmitter.emit('queryBuildEnd', finalStatement, [...variables])
  return finalStatement
}

async function handleColumn (column, nodes, relationships, options) {
        ...
        if (column) {
          if (Array.isArray(column)) {
            for (const item of column) {
              const node = await handleColumn(item, nodes, relationships, options)
              if (node) {
                this.emit('read', node)
              }
            }
          } else {
            const node = await handleColumn(column, nodes, relationships, options)
            if (node) {
              this.emit('read', node)
            }
          }
