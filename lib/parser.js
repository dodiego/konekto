...
function _getFinalQuery (queryObject, cypher, nodes) {
  ...
  if (queryObject._sql) {
    const sqlColumns = []
    const sqlMappings = queryObject._sql
    for (const item of Object.values(sqlMappings)) {
      const columns = [`'konekto_id', ${item.table}.konekto_id`, ...item.columns.map(c => `'${c}', ${item.table}.${c}`)]
...
function sqlInsert (options, node, sqlParams) {
  if (options.sqlMappings && options.sqlMappings[node._label]) {
    ...
    for (const mapping of sqlMappings[node._label].mappings) {
      ...
      sqlColumns.push(mapping.column)
...
function handleSqlInsert (options, node, sqlQuery, sqlParams) {
  const sqlInsertRoot = sqlInsert(options, node, sqlParams)
  if (sqlInsertRoot) {
    sqlQuery.push(sqlInsertRoot)
  }
}
...
module.exports = class extends EventEmitter {
  async jsonToCypherWrite (json, options = {}) {
    ...
    if (graph.root._id) {
      ...
      handleSqlInsert(options, graph.root, sqlQuery, sqlParams)
    ...
    for (let rIndex = 0; rIndex < graph.relationships.length; rIndex++) {
      ...
      if (graph.nodes[r.to]._id) {
        ...
        handleSqlInsert(options, graph.nodes[r.to], sqlQuery, sqlParams)
        if (options.hooks && options.hooks.beforeCreate) {
